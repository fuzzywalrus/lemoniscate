...
Define the canonical layout rules for the Lemoniscate monorepo after the April 2026 flip: which codebase lives where, branch and tag naming conventions, backport workflow, backup-ref guarantees, and the rule that prevents committed binary artifacts. This is the durable reference for how the post-flip repo is organized.
...
### Requirement: Cross-codebase work uses coordinated sibling proposals

When a change affects both modern and PPC codebases, developers SHALL create sibling proposals with identical names — one in `/openspec/changes/<name>/` and one in `/ppc/openspec/changes/<name>/`. Each sibling's `proposal.md` SHALL include a **Sibling** line pointing at the counterpart path so a reader of either proposal can immediately find the other half of the work.

Siblings SHALL remain in sync on scope and naming. Diverging scope between siblings SHALL be recorded by updating both `proposal.md` files in the same PR. A sibling proposal MAY be archived independently if the work on that side is complete or abandoned, but the remaining sibling's `proposal.md` SHALL be updated to reflect that the counterpart has ended.

#### Scenario: A feature lands on both modern and PPC
- **WHEN** a developer plans a feature that requires work on both codebases
- **THEN** two change proposals exist with the same `<name>`, one under each openspec root
- **AND** each `proposal.md` includes a `Sibling: <path>` line referencing the counterpart

#### Scenario: A feature is scoped to one codebase
- **WHEN** a change only affects one codebase
- **THEN** only one proposal exists
- **AND** no sibling reference is required

#### Scenario: One sibling is archived before the other
- **WHEN** the work on one codebase finishes while the other is still in progress
- **THEN** the finished sibling is archived normally under its openspec root's `archive/`
- **AND** the remaining sibling's `proposal.md` is updated to note that its counterpart is complete
