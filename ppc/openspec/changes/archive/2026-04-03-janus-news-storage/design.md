...
**Goals:**
- Parse existing flat text / YAML into structured data for Mnemosyne sync
- Enable Mnemosyne message board sync (`POST /api/v1/sync/msgboard`) by extracting structured posts from `MessageBoard.txt` at sync time
- Read Janus-format directories (`News/`, `MessageBoard.jsonl`) when present, for cross-compatibility
- Keep the existing YAML/flat text backends for all client-facing operations (wire protocol)

**Non-Goals:**
- **REVISED**: Replacing the client-facing backends. The YAML threaded news and flat text message board stay as the primary storage — they produce the correct Hotline wire format. Janus format is read-only for Mnemosyne sync and cross-compat.
- Changing the Hotline wire protocol
- Supporting concurrent writes from multiple server processes
- Auto-migrating from old formats (removed — causes client breakage)
